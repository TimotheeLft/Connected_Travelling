...
uint8_t pwm_g[5] = "PWM_G";
uint8_t pwm_d[5] = "PWM_D";
//user functions

void uart_write(uint8_t *buffer, uint16_t buffer_size){
	for(uint16_t count = 0; count < buffer_size; count++){
		while(app_uart_put(buffer[count]) != NRF_SUCCESS);
	}
}
...
//uint8_t *uart_read(){
//	uint8_t buffer[100];
//	while(app_uart_get(&buffer) != NRF_SUCCESS);
//	return buffer;
//}
